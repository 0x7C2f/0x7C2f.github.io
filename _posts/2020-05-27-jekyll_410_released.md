...
author: Jekyll News
layout: post
permalink: jekyll_410_released.html
original_link: https://jekyllrb.com/news/2020/05/27/jekyll-4-1-0-released/
---
...
- Jekyll now supports rendering excerpts for _pages_ in addition to documents and posts.
- The [`where_exp`](/docs/liquid/filters/#where-expression) filter got enhanced. Earlier, one could just use either `and` or `or` once per expression. Now, one may use those binary operators multiple times in the filter’s expression.
- Jekyll has a new set of filters based on _its flavor_ of the `where` and `where_exp` filters. Named[`find`](/docs/liquid/filters/#find) and [`find_exp`](/docs/liquid/filters/#find-expression) filters respectively, they work similar to their ancestors except that they return **the first object** that satisfies the given conditions.
- Jekyll’s [`number_of_words`](/docs/liquid/filters/#number-of-words) filter can now take an optional argument to better count words of text containing Chinese, Japanese or Korean characters.
...
- The logic for _slugifying_ a given string has been enhanced to support more Unicode characters.
...
- When you build a site with the `--profile` switch, Jekyll will now additionally output a small table showing the amount of time taken during various stages of the _build process_.
